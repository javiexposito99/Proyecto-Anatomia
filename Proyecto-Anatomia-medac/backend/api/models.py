...
class Muestra(Model):
    ...
    interpretacion=ForeignKey(Interpretacion,verbose_name="Interpretación", on_delete=RESTRICT, db_column="interpretacion", to_field="codigo", default='Nulo')
    #imagenes=JSONField(default=list, blank=True)
    #zoom=JSONField(default='x4', validators=[validate_zoom], blank=True)
    def __str__(self):
        return f"{self.codigo}"
class Imagen(Model):
