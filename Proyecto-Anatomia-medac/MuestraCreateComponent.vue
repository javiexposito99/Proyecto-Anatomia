<template>
    ...
    <div>
      ...
      <form @submit.prevent="register">
        <label for="example1" class="form-label" >muestrao: </label>
        <select v-model="muestra" required class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
          <option style="text-align: center;" v-for="item in muestras" :key="item.muestra" :value="item.muestra">{{ item.muestra }}</option>
        </select>
        <div >
  ...
  const muestras = ref([]); // Declarar muestras
  const muestra = ref('');
  const amount = ref('');
  const discount = ref('20');
  const client = ref(localStorage.getItem('username'));
  
  // Obtener la lista de muestraos al montar el componente
  onMounted(async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/muestras/');
      muestras.value = response.data;
    } catch (error) {
      console.error(error);
    }
  });
  ...
  const register = async () => {
    try {
      ...
      await axios.post('http://127.0.0.1:8000/api/muestra/nueva/', {
